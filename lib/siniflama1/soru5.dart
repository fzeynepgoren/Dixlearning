  ...
  void _checkCompletion() async {
    if (fourLegsGroup.length == 2 && twoLegsGroup.length == 2) {
      ...
      try {
        ...
        int previousStars = prefs2.getInt('classification_stage_1_stars') ?? 0;
        if (stars > previousStars) {
          await prefs2.setInt('classification_stage_1_stars', stars);
        } else {
        ...
        if (mounted) {
          ...
          int stars = await _calculateStars();
          // Yıldızları kaydet (roadmap için) - sadece öncekinden daha iyiyse güncelle
          int previousStars = prefs3.getInt('classification_stage_1_stars') ?? 0;
          if (stars > previousStars) {
            await prefs3.setInt('classification_stage_1_stars', stars);
          } else {
            stars = previousStars; // Önceki yıldız sayısını kullan
          }
          _showCompletionDialog(stars);
